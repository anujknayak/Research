...
%X = zeros(size(tVec));X(1:5:end) = 1;
X(end-tRecovery:end) = 0;
k = 0;l = 0;
m = 0;
alphaTmp = 1;
alphaVec(1) = 1;
...
for tInd = 2:length(tVec)
    alphaTmp = exp(-1/tDecay)*X(tInd-1)*alphaTmp + (1-X(tInd-1))*(alphaTmp+m);
    alphaVec(tInd) = min(alphaTmp, 1);
    k = min([(1-X(tInd-1))*(k+1) (1-X(tInd))*(tRecovery-1)]);
    %l = X(tInd);
